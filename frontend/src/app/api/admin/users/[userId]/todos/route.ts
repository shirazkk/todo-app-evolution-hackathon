...
// This API route will validate admin credentials and fetch a specific user's todos from the backend
export async function GET(request: NextRequest, { params }: { params: { userId: string } }) {
  try {
    ...
    // to fetch the user's todos, using admin credentials to authenticate with the backend
    // For now, we'll return a mock response

    // Example of how to make a request to the backend:
    // const backendResponse = await fetch(`${process.env.BACKEND_API_URL}/api/users/${userId}/todos`, {
    //   headers: {
    ...
    // });
    //
    // const userTodos = await backendResponse.json();
    ...
    return new Response(JSON.stringify({ todos: mockTodos }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
