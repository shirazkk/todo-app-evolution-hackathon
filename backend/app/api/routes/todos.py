...
async def create_todo_endpoint(
    ...
    """
    # Verify that the user_id in the URL matches the current user's ID
...
async def get_todos_endpoint(
    ...
    if str(current_user.id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to access todos for this user"
        )
...
async def update_todo_endpoint(
    ...
    if str(current_user.id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to update todos for this user"
        )
...
async def delete_todo_endpoint(
    ...
    if str(current_user.id) != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to delete todos for this user"
        )
